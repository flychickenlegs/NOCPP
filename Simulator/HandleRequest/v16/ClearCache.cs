...
namespace Simulator.HandleRequest.v16
{
    ...
    {
        public async Task<MessageType> ClearCache(Call call)
        {
            string str_retunString = "";

            ClearCacheRequest cleCahReq_payload = new();
            ClearCacheResponse cleCahResp_payload = new();
            ...
            try
            {
                cleCahReq_payload = JsonSerializer.Deserialize<ClearCacheRequest>(call.Payload);

                // Write down your logic here.
                if (true)
                {
                    Console.WriteLine("ClearCache succeed.");
                    cleCahResp_payload.Status = ClearCacheResponseStatus.Accepted;
                }
                else
                {
                    Console.WriteLine("ClearCache failed.");
                    cleCahResp_payload.Status = ClearCacheResponseStatus.Rejected; 
                }
                callResult.Payload = Utility.CalssToJsonElement(cleCahResp_payload);
