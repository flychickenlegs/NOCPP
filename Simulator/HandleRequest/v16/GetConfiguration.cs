...
namespace Simulator.HandleRequest.v16
{
    ...
    {
        public async Task<MessageType> GetConfiguration(Call call)
        {
            string str_retunString = "";

            GetConfigurationRequest getConfReq_payload = new();
            GetConfigurationResponse getConfResp_payload = new();
            ...
            try
            {
                getConfReq_payload = JsonSerializer.Deserialize<GetConfigurationRequest>(call.Payload);

                // Write down your logic here.
                if (true)
                {
                    Console.WriteLine("GetConfiguration succeed.");
                    getConfResp_payload.ConfigurationKey = [];
                    getConfResp_payload.UnknownKey = [];
                }
                else
                {
                    Console.WriteLine("GetConfiguration failed.");
                }
                callResult.Payload = Utility.CalssToJsonElement(getConfResp_payload);
